...
- 自分の **GitHub リポジトリ**を作り、原稿（`.tex` と `.pdf`）を置く。
- **必ず private（非公開）にする。public にしないこと。** 未公開の研究内容が外部に見えてしまいます（毎年うっかり public のままの人がいます。作成時と、心配なら設定画面で一度確認を）。
- リポ名は **できれば `名前_コンテンツ名`**（例：`fujita_abstract` / `yoshida_thesis`）にしてください。久住側でまとめて扱うときに楽になります（後から名前を直すのは手間なので、最初にこの形で）。
- **久住を collaborator に招待**する。private なので、招待がないと久住はリポも Issue も見られません。
...
> リポ**内**の構成やファイル名は自由です（上のリポ名の推奨は別）。ひとつだけ、**「久住側でも開いて読める・ビルドできる状態」で共有**してください。必要なスタイルファイルなどが欠けていると、こちらでも確認できず止まってしまいます。
